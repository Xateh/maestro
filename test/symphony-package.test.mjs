...
import { resolveWorkspaceLocalInvocation } from "../bin/symphony.mjs";
import { parseCliArgs } from "../src/workflow.mjs";

// PACKAGE_ROOT is bin/../ == the symphony repo root
const PACKAGE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

test("package contains the standalone Symphony implementation", async () => {
  const entry = await readFile(new URL("../bin/symphony.mjs", import.meta.url), "utf8");

  assert.match(entry, /from "\.\.\/src\/workflow\.mjs"/);
  assert.doesNotMatch(entry, /digital-twin-research/);
  assert.doesNotMatch(entry, /from "\.\/symphony\//); // no old-layout imports
  await access(new URL("../src/workflow.mjs", import.meta.url));
  await access(new URL("../src/tui.mjs", import.meta.url));
});

test("package.json declares Symphony runtime dependencies", async () => {
...
test("Symphony modules load from package paths", () => {
  const parsed = parseCliArgs(["node", "bin/symphony.mjs", "ops/WORKFLOW.md", "--port", "0"]);
...
test("local commands keep central state and use caller cwd", () => {
  ...
  assert.deepEqual(invocation.args, [
    "tui",
    "--state-dir",
    path.join(PACKAGE_ROOT, ".symphony"),
  ]);
});

test("unblock commands also use central Symphony state", () => {
