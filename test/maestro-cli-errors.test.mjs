...
test("local command in an uninitialized dir falls back but recommends init", async () => {
  await withTempDir(async (dir) => {
    const { stderr } = await runCli(["status"], { cwd: dir });

    // PR #11 keeps the package-default fallback, but no longer does it
    // silently: it points the user at `maestro init`.
    assert.match(stderr, /^maestro: no \.maestro\/ found here/m);
    assert.match(stderr, /maestro init/);
    assert.doesNotMatch(stderr, /maestro_failed/);
    // the fallback never scaffolds .maestro/ in the caller's dir
